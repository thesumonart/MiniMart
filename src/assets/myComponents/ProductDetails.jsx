...
const ProductDetails = () => {
  ...
  return (
    <div className="">
      <div className="flex flex-col md:flex-row gap-4 w-full py-10 px-4">
        <div className="w-full md:w-1/2">
          <figure className="flex w-full rounded-md overflow-hidden">
            <img className="w-full" src={product.image} alt={product.name} />
          </figure>
        </div>
        <div className="w-full md:w-1/2 flex flex-col gap-4">
          <h1 className="text-2xl font-bold md:text-3xl">{product.name}</h1>
          <p className="text-base text-gray-500 font-">{product.shortDesc}</p>
          ...
          <p className="text-2xl font-bold font-sans">{`$${product.price}`}</p>
          <div className="flex gap-2">
            <Button
              variant="secondary"
